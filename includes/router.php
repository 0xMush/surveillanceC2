...
function routeRequest(): void {
    ...
    $authOnly = ['csrf', 'beacons', 'tasks', 'results', 'files', 'media',
                 'persons', 'person', 'person_photo_get', 'human_files', 'human_file_read',
                 'device_info', 'terminal', 'ls_device', 'device_read', 'browse_cache',
                 'payloads', 'payload'];
    ...
    if (in_array($action, $public)) {
        // no auth needed
    } elseif (in_array($action, $authOnly)) {
        requireAuth();
    } else {
        requireAuth();
        validateCsrf();
